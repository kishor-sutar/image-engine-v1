...
const TRANSFORMED_DIR = path.join(STORAGE_ROOT, "transformed");
...
const storage = {
    ...
    async loadSource(userId: string, fileName: string): Promise<Buffer> {
    ...
    async getFromCache(key: string): Promise<Buffer | null> {
        ...
        // 2️ Disk async check
        const diskPath = path.join(TRANSFORMED_DIR, key.replace(/\//g, "_"))
...
async function loadSource(fileName: string): Promise<Buffer> {
    const localPath = path.join(ORIGINALS_DIR, fileName);
...
export const handler = async (
    ...
    try {
        ...
        // Expected format: u/:userId/:image
        ...
        const transformPromise = (async () => {
            ...
            const source = await storage.loadSource(userId,fileName);
